...
# =========================
# VERIFY LINUX_PATCHES_DIR IS WRITABLE
# =========================
os.makedirs(LINUX_PATCHES_DIR, exist_ok=True)
test_file = os.path.join(LINUX_PATCHES_DIR, ".write_test")
try:
    with open(test_file, "w") as f:
        f.write("test")
    os.remove(test_file)
    print(f"✅ Destination writable: {LINUX_PATCHES_DIR}")
except Exception as e:
    print(f"❌ Cannot write to destination: {LINUX_PATCHES_DIR}")
    print(f"   Error: {e}")
    print(f"   Fix: sudo chmod -R 777 {LINUX_PATCHES_DIR}")
    exit(1)
...
# Ubuntu version → codename (needed for MongoDB repo)
codename_map = {
    "20.04": "focal",
    "22.04": "jammy",
    "24.04": "noble",
...
# CHANGED 20-03-2026 14:06 — package repo detection
# Add package name patterns here for custom repos
...
# CHANGED 20-03-2026 14:06 — detect which repo a package needs
...
def download_patch(agent_id, patch_id, row):
    ...
    if is_already_downloaded(final_dest):
        ...
        print(f"{prefix} ⏭️  SKIPPED → Already downloaded ({len(existing)} files)")
    ...
    # CHANGED 20-03-2026 14:06 — repo type detect karo
    ...
    print(f"{prefix} 🚀 {actual_package} | {image} | repo: {repo_type}")
    print(f"{prefix} 📥 Temp  : {output_path}")
    print(f"{prefix} 📦 Final : {final_dest}")

    # -------------------------------------------------------
    # DOCKER COMMAND — CHANGED 20-03-2026 14:06
    # -------------------------------------------------------
...
"""
    ...
    if repo_type == "mongodb":
        # CHANGED 20-03-2026 14:06 — MongoDB official repo setup
        docker_cmd = f"""
docker run --rm \
  -v {output_path}:/out \
  -w /out \
...
echo "--- Step 3: download {actual_package} ---"
cd /out
if apt-get download {actual_package} 2>&1; then
    echo "OK: {actual_package} downloaded"
else
    echo "WARN: {actual_package} not found — trying mongodb-org-server..."
    apt-get download mongodb-org-server 2>&1 || true
fi
...
"""
        ...
        # ntopng — standard ubuntu universe repo mein hai
        docker_cmd = f"""
docker run --rm \
  -v {output_path}:/out \
  -w /out \
...
"""
    ...
    # =========================
    # FAILED — cleanup temp, do NOT create final_dest
    # =========================
    if not patch_files:
        print(f"{prefix} ❌ FAILED → No .deb files downloaded")
        cleanup_folder(output_path)
        print(f"{prefix} 🧹 Temp folder cleaned up — no folder created in linux_patches")
        return agent_id, patch_id, "FAILED", [], None

    # =========================
    # SUCCESS — create final_dest only now and move files
    # =========================
    os.makedirs(final_dest, exist_ok=True)

    print(f"\n{prefix} 📂 Moving to: {final_dest}")
    moved_files = []
    ...
    print(f"{prefix} ✅ SUCCESS → {len(moved_files)} file(s) moved to {final_dest}")
