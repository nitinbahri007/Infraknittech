...
def upload_packages():
    ...
    # ===============================
    # OS KE HISAAB SE FOLDER SELECT
    # ===============================
    os_name = request.form.get("os_name", "").strip().lower()

    if "red hat" in os_name or "redhat" in os_name or "rhel" in os_name:
        upload_folder = "redhat_patches"
    elif "ubuntu" in os_name or "debian" in os_name:
        upload_folder = "ubuntu_patches"
    else:
        upload_folder = "other_patches"
    ...
    try:
        with open(path, "r") as f:
            total_packages = len([l for l in f.readlines() if l.strip()])

        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO patch_alert (agent_id, kb, message, category, created_at)
            VALUES (%s, %s, %s, %s, NOW())
        """, (
            agent_id,
            "PACKAGE_LIST",
            f"Package list received. Total packages: {total_packages}",
            upload_folder   # "redhat_patches" ya "ubuntu_patches"
        ))
        conn.commit()
        cursor.close()
        conn.close()
        print(f"✅ patch_alert log created — {total_packages} packages — {upload_folder}")
    except Exception as e:
        print(f"❌ patch_alert insert failed: {e}")
