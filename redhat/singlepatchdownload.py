...
def fetch_package(patch_id: str, agent_id: str, logger: logging.Logger) -> dict | None:
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT package_name, version, repo
...
echo '[INFO] Downloading {pkg_name}-{pkg_ver} ...'
dnf download \\
  --destdir=/repo \\
  --repo='{pkg_repo}' \\
  '{pkg_name}-{pkg_ver}'
...
def main():
    ...
    # Step 2: Destination folder (baseos ya appstream)
    dest = DOWNLOAD_DIR / ("appstream" if "appstream" in pkg_repo else "baseos")
